...
#   stage B: BDF2 + MUSCL, dt=1e-7, pseudo-CFL 10, 30 inner  (validated settings)
...
status "case=$CASE mesh=$MESH seed=$(basename $SEED) steps=$NSTEP np=$NP dt=$DT"
...
cfg A.cfg STAGE=A READ_BINARY=NO RESTART_ITER=1 SOLUTION=seed \
    TIME_MARCHING=DUAL_TIME_STEPPING-1ST_ORDER TIME_ITER=4 INNER_ITER=50 CFL=0.5 MUSCL=NO \
    WRT_RESTART=1 WRT_VOL=1000000 WRT_SURF=1000000
run A A.cfg
cfg B.cfg STAGE=B READ_BINARY=YES RESTART_ITER=4 SOLUTION=restart \
    TIME_MARCHING=DUAL_TIME_STEPPING-2ND_ORDER TIME_ITER=$((NSTEP+1)) INNER_ITER=30 CFL=10.0 MUSCL=YES \
